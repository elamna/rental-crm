...
// Заготовки: пустой лист — худшее начало для документа
rep(
  "const VARIABLES = [",
  L(
    "/**",
    " * Готовые каркасы. Составить акт с нуля в редакторе долго и муторно, а формы",
    ...
    '      "<p>1.1. Арендодатель передаёт Арендатору во временное владение и пользование оборудование:</p>",',
    "      \"{{items_table}}\",",
    '      "<p>1.2. Срок аренды: с {{rental_start}} по {{rental_end}} ({{rental_days}}).</p>",',
    '      "<h3>2. Стоимость и порядок расчётов</h3>",',
...
// Поиск по переменным и вставка заготовки
rep(
  L(
...
rep(
  ...
  L(
    ...
    "  }",
    "",
    "  /** Поиск по переменным: список длинный, листать его глазами неудобно */",
    ...
    "    if (hasContent && !confirm(\"Заготовка заменит текущее содержимое шаблона. Продолжить?\")) return;",
    "    editor.commands.setContent(body);",
    "    bodyRef.current = editor.getHTML();",
    '    if (isNew && (name === "Новый шаблон" || !name.trim())) setName(blueprintName);',
    "  }"
  )
);

// Кнопки в шапке редактора: заготовки и предпросмотр печати
rep(
  L(
...
// Поле поиска в панели переменных
rep(
  L(
    '            <p className="text-[13px] font-bold uppercase tracking-wide text-[var(--color-text-muted)]">Переменные для шаблона</p>',
    '            <p className="mt-0.5 text-[13px] text-[var(--color-text-muted)]">Поставьте курсор → нажмите +</p>'
  ),
  L(
    '            <p className="text-[13px] font-bold uppercase tracking-wide text-[var(--color-text-muted)]">Переменные для шаблона</p>',
    '            <p className="mt-0.5 text-[13px] text-[var(--color-text-muted)]">Поставьте курсор → нажмите +</p>',
...
// Рендер по отфильтрованным группам, при поиске всё раскрыто
...
rep(
  ...
  L(
    "          ))}",
    "          {groups.length === 0 && (",
    '            <p className="px-4 py-6 text-center text-[13px] text-[var(--color-text-muted)]">Ничего не нашлось</p>',
    "          )}",
    "        </div>",
    "      </div>",
    "    </div>",
