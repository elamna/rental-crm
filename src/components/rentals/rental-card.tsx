  ...
  const body = (
    <>
      ...
      <div className={cn("p-3.5 text-white", headerBg)}>
        <div className="flex items-start justify-between gap-2">
          <span className="flex min-w-0 items-center gap-2">
            ...
            <span className={cn("truncate rounded-full bg-white px-2.5 py-1 text-[11.5px] font-bold", chipText)}>
              {label}
            </span>
          </span>
          <span className="flex shrink-0 items-center gap-1 text-[12.5px] font-semibold text-white/90">
        ...
        <div className="mt-3 grid grid-cols-2 gap-2">
          <div className={cn("rounded-[10px] px-2.5 py-1.5", st.tile)}>
            <div className="flex items-center gap-1 text-[10.5px] font-medium text-white/85">
              Дата начала <History className="h-2.5 w-2.5" />
            </div>
            <div className="text-[12.5px] font-bold">{formatDateTimeDisplay(rental.startAt ?? "") || "—"}</div>
          </div>
          <div className={cn("rounded-[10px] px-2.5 py-1.5", st.tile)}>
            <div className="flex items-center gap-1 text-[10.5px] font-medium text-white/85">
              Дата конца <History className="h-2.5 w-2.5" />
            </div>
            <div className="text-[12.5px] font-bold">{formatDateTimeDisplay(rental.endAt ?? "") || "—"}</div>
          </div>
        </div>

        <div className={cn("mt-2 flex items-center gap-2.5 rounded-[10px] px-2.5 py-2", st.tile)}>
          <div className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-white/85 text-[11px] font-bold text-[#3B2E4A]">
            {rental.client.name
              .split(" ")
              .slice(0, 2)
              .map((n) => n[0])
          ...
          <div className="min-w-0">
            <div className="truncate text-[13.5px] font-bold">{rental.client.name}</div>
            <div className="flex items-center gap-1 text-[11.5px] text-white/85">
      ...
      <div className="p-4">
        ...
        <div className={cn("mt-2.5 rounded-[10px] px-2.5 py-2 text-center text-[12.5px] font-semibold", pay.bg, pay.text)}>
        ...
        <div className="mt-3 rounded-[10px] border border-[var(--color-border)] px-2.5 py-2">
          ...
          {rental.items.length === 0 && (
            <div className="text-[11.5px] text-[var(--color-text-muted)]">Позиции не добавлены</div>
          )}
