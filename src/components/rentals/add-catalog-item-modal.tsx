    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/30 sm:items-center sm:p-4" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="flex w-full max-w-md flex-col rounded-t-[20px] border border-[var(--color-border)] bg-[var(--color-surface)] card-shadow safe-bottom sm:rounded-[var(--radius-card)]" style={{ maxHeight: "88dvh" }}>
        ...
        <div className="flex items-center justify-between p-5 pb-3 shrink-0">
          <h3 className="text-[16px] font-semibold">Добавить товар из каталога</h3>
                ...
                <div key={item.id} className={`rounded-[10px] border transition ${isSelected ? "border-[var(--color-primary)] bg-[var(--color-primary-soft)]" : "border-[var(--color-border)] hover:bg-[var(--color-bg)]"}`}>
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-[12.5px] text-[var(--color-text-muted)]">{item.sku || "без артикула"}</div>
                    </div>
                    <span className="shrink-0 text-[13.5px] font-semibold">{formatMoney(item.rentalPricePerDay)}/сутки</span>
                  </button>
