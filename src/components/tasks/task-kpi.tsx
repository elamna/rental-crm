...
export function TaskKpi() {
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? (
        <p className="py-10 text-center text-[14px] text-[var(--color-text-muted)]">Считаем…</p>
      ) : rows.length === 0 ? (
        <p className="py-10 text-center text-[14px] text-[var(--color-text-muted)]">Нет данных за период</p>
      ) : (
            ...
            <div key={r.userId} className="rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-surface)] p-4 card-shadow">
              <div className="mb-3 flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="truncate text-[15px] font-semibold">{r.userName}</div>
...
function ScoreBadge({ score }: { score: number }) {
  ...
  return <span className={cn("shrink-0 rounded-[8px] px-2.5 py-1 font-display text-[16px] font-bold", cls)}>{score}</span>;
}
...
function Metric({ label, value, tone }: { label: string; value: string; tone?: "bad" }) {
  return (
    <div className="rounded-[8px] bg-[var(--color-bg)] px-2.5 py-1.5">
      ...
      <div className={cn("text-[14px] font-semibold", tone === "bad" && "text-[#C0272D]")}>{value}</div>
    </div>
  );
}
