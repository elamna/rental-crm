...
export async function GET(req: NextRequest) {
  ...
  `).all(from, to) as {
    ...
    paid: number; total: number; status: string; created_at: string; paid_at: string | null;
  ...
  for (const r of rentals) {
    ...
    const expenses = JSON.parse(r.expenses_json || "[]") as { amount: number; type?: string; description?: string; createdAt?: string }[];
    const deposit = r.deposit_json ? JSON.parse(r.deposit_json) as { amount?: number } : null;

    if (r.paid > 0) {
      totalIncome += r.paid;
      transactions.push({
        id: `pay_${r.id}`, date: r.paid_at ?? r.created_at, type: "income",
    ...
    for (const e of expenses) {
      ...
      transactions.push({
        ...
        description: e.description ?? e.type ?? `Расход по аренде №${r.number}`,
        rentalNumber: r.number, clientName, rentalId: r.id,
      });
    }
  ...
  for (const t of transactions) {
    ...
    // Штраф — начисление, а не поступление денег: в графике его нет, как и в KPI
