...
import { BLUEPRINTS } from "@/lib/document-blueprints";
            ...
            {templates.map((t) => (
              <div key={t.id} className="flex items-center justify-between rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-3 card-shadow">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => { setEditing(t); setIsNew(false); }} className="flex items-center gap-1.5 rounded-[8px] border border-[var(--color-border)] px-3 py-1.5 text-[13.5px] hover:bg-[var(--color-bg)]"><Pencil className="h-3.5 w-3.5" /> Редактировать</button>
      ...
      <header className="flex items-center justify-between border-b border-[var(--color-border)] bg-[var(--color-surface)] px-5 py-3 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <input
            ...
            className="min-w-0 flex-1 border-b border-transparent pb-0.5 text-[17px] font-bold outline-none placeholder:text-[var(--color-text-muted)] focus:border-[var(--color-primary)] sm:min-w-[260px] sm:flex-none"
          />
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="rounded-[10px] bg-[var(--color-primary)] px-5 py-2 text-[14px] font-semibold text-[var(--color-on-primary)] disabled:opacity-50 hover:bg-[var(--color-primary-hover)]"
          >
          ...
          {groups.map((group) => (
            <div key={group.group}>
              <button
                ...
                className="flex w-full items-center justify-between px-4 py-2.5 text-[13px] font-semibold text-[var(--color-text)] hover:bg-[var(--color-bg)] border-b border-[var(--color-border)]"
              >
                {group.group}
