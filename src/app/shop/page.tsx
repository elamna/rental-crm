        ...
        {loading ? (
          ...
          <div className="overflow-x-auto rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-surface)] card-shadow">
            <table className="w-full min-w-[720px] text-[13px]">
              <thead>
                <tr className="border-b border-[var(--color-border)] bg-[var(--color-bg)] text-left text-[12px] text-[var(--color-text-muted)]">
                ...
                {products.map((p) => (
                  <tr key={p.id} className="border-b border-[var(--color-border)] last:border-0 hover:bg-[var(--color-bg)]">
                    <td className="px-4 py-3">
                      ...
                      {p.category && <div className="text-[11.5px] text-[var(--color-text-muted)]">{p.category}</div>}
                    </td>
                    <td className="px-4 py-3 font-mono text-[12px] text-[var(--color-text-muted)]">{p.sku || "—"}</td>
                    <td className="px-4 py-3 font-mono text-[12px] text-[var(--color-text-muted)]">{p.serialNumber || "—"}</td>
      ...
      >
        <div className="mb-5 flex items-start justify-between">
          <div>
            ...
            <p className="text-[12.5px] text-[var(--color-text-muted)]">Продаётся безвозвратно, в аренду не выдаётся</p>
          </div>
  ...
  return (
    <label className={cn("block", className)}>
      <span className="mb-1.5 block text-[12.5px] font-medium text-[var(--color-text-muted)]">
        ...
        {hint && <span className="ml-1 text-[11.5px] opacity-70">({hint})</span>}
      </span>
      {children}
    </label>
  );
